...
| [`_applyArchiveFilter`](#_applyarchivefilter) | 方法（`_ManagementPageState`） | A | 把列表收窄到所选的本地存档筛选和分类筛选。 |
...
- **用途：** 把候选列表收窄到 AppBar 中当前选择的本地存档筛选和分类筛选，使一条规则服务本页全部三个列表构建器。
- **输入：** `animes` — 未筛选的候选（目前总是 `_allAnime`）。
- **返回：** `List<Anime>`。
- **副作用：** 无（读取 `_archiveFilter`、`_categoryFilter`、`_insights` 和 `appSettingsProvider`）。
- **算法：** 1) 对私有 `_ArchiveFilter` 枚举做 `switch`——`all` 原样返回输入，`archived` 保留
  `a.localArchive?.archived == true`，`notArchived` 保留其补集（`!= true`）。2) 当设置了 `_categoryFilter` **且**
  `autoCategoriesEnabled` 开启时，只保留 [`resolveCategories`](../../categories/services/category_service.md#resolvecategories)
  得出的 id（自己的、映射的或 AI 的，使用 `_insights`）中包含它的记录。
- **用法：**
  ```dart
...
- **备注：** `notArchived` 刻意把"从未记录"（`localArchive == null`）和"明确记录为未保留"
  ...
  到 `storage_config.json`，外壳每次重建本页时都会重置为 `all`。分类筛选（`_categoryFilter`，`null` 表示全部）
  同样只是视图状态，由 AppBar 中一个仅在自动分类开启时出现的 `PopupMenuButton` 设置（先是*全部分类*，然后是分类表中
  每个 id 及其 [`categoryLabel`](category_widgets.md#categorylabel)）。关闭自动分类会隐藏该按钮，并让筛选不再生效，
  即使仍保存着某个值。
