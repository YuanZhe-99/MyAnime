...
`AiSettingsTiles` 于 1.6.0（M3）新增，构建计划中的「分类与推荐」设置分区里的端侧 AI 行：「使用端侧 AI」开关、
带操作的模型状态行、「使用更快的模型」（Android，仅当两种尺寸都有提供时）、关于模型归属的说明，以及一个折叠的
「技术详情」。在 Windows、Linux 和 Web 上它什么也不渲染。**它尚未放入 `settings_page.dart`：** 在 M4 加入让 AI
开关有事可做的自动分类开关之前，该分区在 `master` 上保持隐藏，因此在 `master` 上只有
`test/ai_settings_tiles_ui_test.dart` 会用到它。见 [`../services/on_device_ai_service.md`](../services/on_device_ai_service.md)
...
- **用法：** `test/ai_settings_tiles_ui_test.dart`（用 `debugDefaultTargetPlatformOverride` 按平台测试）；从 M4
  起用于设置页。
