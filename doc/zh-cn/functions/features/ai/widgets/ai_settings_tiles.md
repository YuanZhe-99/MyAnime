...
「技术详情」。在 Windows、Linux 和 Web 上它什么也不渲染。自 1.6.0（M4）起，`settings_page.dart` 把它放在*自动分类*开关之后，
并以该开关作为 `featuresOn:`，因此只有在自动分类开启时才能打开 AI 开关；关闭自动分类也会关闭 AI。见 [`../services/on_device_ai_service.md`](../services/on_device_ai_service.md)
...
- **用法：** `settings_page.dart` 的「分类与推荐」分区（`AiSettingsTiles(featuresOn: settings.autoCategoriesEnabled)`）；
