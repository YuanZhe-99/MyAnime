...
**总计：** 仓库的 `/// Purpose:` 注释数为 **853**（按 `AGENTS.md` 中的函数解释层约定，排除生成的 `lib/l10n/` 代码——见 [l10n/INDEX.md](l10n/INDEX.md)）。下方各行合计 **843** 个已记录声明。

| Tier | 计数 |
|---|---|
| Tier A（完整条目：Purpose/Inputs/Returns/Side effects/Algorithm/Usage/Notes） | 530 |
| Tier B（仅索引行） | 313 |
| **总计** | **843** |
...
这些总计在 1.4.0 中已对照真实源码树重新计算，1.5.0 中再次重算（在四个新文件与五个改动文件中新增 93 个声明），1.5.1 中又一次重算（在五个改动文件中新增 24 个声明），1.5.2 中再次重算（在一个新文件与四个改动文件中新增 15 个声明），1.5.3 中再次重算（在三个新文件与六个改动文件中新增 22 个声明），1.5.4 中再次重算——本次在三个改动文件中新增 11 个声明，并把另外三行从 Tier B 提升到 Tier A，没有新增任何文件。1.4.0 之前的数字（682 个 `Purpose:` 注释与 685 个已记录声明）与源码以及本文件自身的逐文件行都已严重偏离——当时逐文件行合计仅为 615。若要改动这些数字，请测量而不要手工调整：
