...
由于合并判断「是否变化」**只**看 `modifiedAt` 与基线的先后，一次动了它的后台写入将与真实编辑无法区分，
从而会：

- 让在另一台设备上删除的记录复活（本地看起来被修改过，于是合并保留它），以及
- 为用户从未做过的编辑引发冲突。

不动它则 `localChanged` 恒为 false，两者都不会发生。刷新到的数据仍然会到达其他设备 —— 远端没有碰过这条
记录时，合并保留本地副本，而上传与否是按文件原文比较决定的，内容变了照样上传。当远端**确实**改动了这条
记录时，远端获胜、缓存更新被丢弃，这对缓存而言是正确的：更新器之后会重新抓取。

应用一条更新**建议**则是相反的情况，它确实会更新 `modifiedAt` —— 那是用户的编辑。
...
- **合并从不产生冲突。**垃圾箱是相对于基线合并的集合：只在一侧存在的条目，若基线中没有则保留（它在那一侧被移入垃圾箱），
  ...
  传入。
- **钉选与续作资料（1.6.3）**保持这一性质。钉选——全局的、每部缺失续作的以及每个相关推荐列表的——都是集合，合并方式与
  垃圾箱完全相同。缺失续作抓取到的简介和缩略图是以续作为键的缓存：两侧都存在时 `fetchedAt` 较晚的胜出，在一侧被删除
  （因移入垃圾箱或修剪）的则保持删除。合并之后恢复两条规则：一张卡片最终既被钉选又在垃圾箱中——一台设备钉选了它，而另一台
  换一批把它移走——时保持**钉选**；移入垃圾箱的续作失去其资料。1.6.2 构建原样携带这些新键。
