...
| `recommendations_page.dart` | Yes | Through `listColumnCount` with **Home's column preference** (`homeListColumns`); the page has no column button of its own (1.6.0). Pushed outside the shell, so it measures the raw window: the screen width is the content width. |
